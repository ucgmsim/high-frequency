...
//! Two notes on method:
//!
//! * Kernels that draw from the RNG (`stoc_f`, `radfrq_lin`,
//!   `normal_random_number`) advance the generator across iterations, so
//!   successive iterations see different values. The *work* is identical
//!   regardless — the only value-dependent branch anywhere is
//!   `normal_random_number`'s zero-rejection retry, which fires with probability
...
//!   ray topology `gf_amp_tt` actually produces) rather than to whatever is
...
use hb_high::ray::{cagcon, cr, dtdp, geom_terms, gf_amp_tt, pnot, trav, ttime};
...
/// A ray in the shape `gf_amp_tt` builds for `itype=1`: segments running from the
...
/// `/travel/` and `/coff/` filled by the real `trav`, so the downstream ray
/// kernels see self-consistent state.
fn ray_state_after_trav(ksrc: usize, v: &Vmod) -> RayState {
...
fn spectrum(np2: usize) -> Array1<Complex32> {
    ...
    for i in 1..=np2 {
        cw[i] = Complex32::new(g.rand_numb() - 0.5, g.rand_numb() - 0.5);
    }
    cw
}
...
fn bench_rng(c: &mut Criterion) {
    ...
    group.bench_function("rand_numb", |b| {
        let (mut g, _) = Pcg32::seed(1);
        b.iter(|| black_box(g.rand_numb()))
    });

    // normal_random_number is called once per stoc_f, i.e. three times per
    ...
    for &n in &[1024usize, 4096, 65536] {
        ...
        group.bench_with_input(BenchmarkId::new("normal", n), &n, |b, &n| {
            ...
            b.iter(|| normal_random_number(&mut g, black_box(n), &mut acc))
        });
        group.bench_with_input(BenchmarkId::new("ranu2", n), &n, |b, &n| {
            let (mut g, _) = Pcg32::seed(1);
            let mut rn = Array1::<f32>::new(n);
            b.iter(|| ranu2(&mut g, black_box(n), &mut rn))
        });
    }
    group.finish();
}
...
/// `radfrq_lin` is the second-largest cost after the FFT and the largest RNG
/// consumer: `nr = 1000` iterations, each drawing five deviates and calling
/// `rdatn`, so 5,000 draws and 5,000 `rdatn` calls per invocation — twice per
/// subfault per ray. `rdatn` is benched separately so the split is visible.
fn bench_radiation(c: &mut Criterion) {
    let mut group = c.benchmark_group("radiation");

    group.throughput(Throughput::Elements(1));
    group.bench_function("rdatn", |b| {
        b.iter(|| {
            black_box(rdatn(
                black_box(1.2),
                black_box(0.9),
    ...
    group.throughput(Throughput::Elements(1));
    group.bench_function(BenchmarkId::new("radfrq_lin", format!("nr{NR}")), |b| {
    ...
    ranu2(&mut g, NR, &mut rna);
    ranu2(&mut g, NR, &mut rnb);
    group.bench_function(BenchmarkId::new("radv_lin", format!("nr{NR}")), |b| {
        b.iter(|| {
            radv_lin(1.2, 0.9, -0.4, 2.1, 2.6, &dfr, nfold, &rna, &rnb, NR, &mut rdna)
        })
    });
...
/// `pnot` is interesting because it always bisects and always exits on the
/// `|dtau/dp| <= 0.01` tolerance rather than the 40-iteration cap, and every
/// iteration calls `dtdp`, which calls `cr` once per layer. So its cost is
/// `iterations x layers x cr`.
fn bench_ray(c: &mut Criterion) {
    ...
    group.throughput(Throughput::Elements(1));
    group.bench_function("cr", |b| {
        b.iter(|| black_box(cr(black_box(p), black_box(3.2))))
    });
    group.bench_function("cagcon", |b| {
        b.iter(|| black_box(cagcon(&st, &v, black_box(p), 1, black_box(60.0))))
    });
    group.bench_function("dtdp", |b| {
        b.iter(|| black_box(dtdp(&st, &v, black_box(p), 1, black_box(60.0))))
    });
    group.bench_function("pnot", |b| {
        b.iter(|| black_box(pnot(&st, &v, 1, black_box(60.0))))
    });
    group.bench_function("ttime", |b| {
        b.iter(|| black_box(ttime(&st, &v, 1, black_box(0.15), 0.0, black_box(60.0))))
    });
    group.bench_function("geom_terms", |b| {
        b.iter(|| black_box(geom_terms(&st, &v, black_box(30.0), black_box(0.15), 1)))
    });
    group.bench_function("trav", |b| {
        b.iter_batched_ref(
            || ray_state(18),
            |st| trav(st, &v, 1, black_box(30.0), black_box(v.thic[1])),
            criterion::BatchSize::SmallInput,
        )
    });
    // The whole cluster, as the main program calls it once per subfault per ray.
    group.bench_function("gf_amp_tt", |b| {
        b.iter_batched_ref(
            RayState::default,
            |st| {
                black_box(gf_amp_tt(
...
fn bench_spectrum(c: &mut Criterion) {
    ...
    for &np2 in NP2S {
        ...
        // stoc_f: one FFT plus np2 normal draws plus the per-bin spectral shape.
        group.bench_with_input(BenchmarkId::new("stoc_f", np2), &np2, |b, &np2| {
        ...
        // highcor_f: the radiation multiply, one inverse FFT, the scale and the
        // raised-cosine taper.
        group.bench_with_input(BenchmarkId::new("highcor_f", np2), &np2, |b, &np2| {
            let mut stdd = Array1::<f32>::new(np2);
            b.iter_batched_ref(
                || src.clone(),
                |cw| highcor_f(nf, mf, np2, cw, &mut stdd, &rdna),
                criterion::BatchSize::SmallInput,
            )
        });

        group.bench_with_input(BenchmarkId::new("siteamp", np2), &np2, |b, &np2| {
            b.iter_batched_ref(
                || src.clone(),
                |cw| siteamp(np2, cw, &dfr, 20, &fn_, &an),
                criterion::BatchSize::SmallInput,
            )
        });

        group.bench_with_input(BenchmarkId::new("flzero", np2), &np2, |b, &np2| {
    ...
    group.throughput(Throughput::Elements(1));
    group.bench_function("get_sitefacs", |b| {
        b.iter(|| get_sitefacs(&v, black_box(20), black_box(20), &fn_, &mut an))
    });
...
/// `even_dist2` runs once per segment per station and calls `DELAZ5` once per
