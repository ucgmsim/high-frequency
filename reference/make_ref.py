...
def main():
    ...
    # Also emit the subprograms without the main program, so kernel drivers in
    # harness/kernels/ can link against the real Fortran routines and produce
    # goldens for a single subprogram at a time.
    split = next(i for i, ln in enumerate(lines)
                 if ln.lstrip().startswith("subroutine distazi"))
    if "END" not in lines[split - 2]:
        sys.exit(f"guard failed: expected the main program's END just before "
                 f"'subroutine distazi' at line {split + 1}, got "
                 f"{lines[split - 2].strip()!r}")
    subs = HERE / "hb_high_subs.f"
    subs.write_text(
        "c Generated by make_ref.py -- subprograms of hb_high_ref.f with the\n"
        "c main program removed, for linking into single-kernel test drivers.\n"
        "c Do not edit; edit hb_high_orig.f's vendored copy or make_ref.py.\n"
        + "".join(lines[split:])
    )
    print(f"wrote {subs} ({len(subs.read_text().splitlines())} lines)")
