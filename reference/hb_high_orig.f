c hb_high_v5.0
c Needs to be compiled with 'gfortran' (won't work with g77) (Edit: can be compiled with xlf too SEB)

c
c 20200106 JMP
c 1) updated to version 5.4.6
c

c
c 17/03/2015 SEB:
c
c  1) Modification made to be compatible with xlf - all rand() replaced by rand_numb()
...
c      fce=zz*vr/dlm/pai
#if defined (VERSION2) || defined (VERSION3) || defined (VERSION4)
      include 'params.h'
#else
      include 'params_no_window.h'
#endif
...
c 20141219 RWG: changed c0 to 2.1 and added Czero input parameter replacing fcfac
...
#ifdef BINMOD
      ! START MOD no individual files
      open(22, file = outname(1:loc1),form='unformatted',access='stream')
      CALL FSEEK(22, seek_bytes, 0)
      ! END MOD
#endif
...
3461        continue
            ...
            twin(i,j) = 2.12*(tw0 + dpath)*dexp(pd_pert)
#else
            twin(i,j) = tw0 + dpath
#endif
#if defined (VERSION2) || defined (VERSION3) || defined (VERSION4)
            if(twin(i,j).gt.81.92) twin(i,j) = 81.92
#endif
...
8461     continue
 ...
 777  continue
      ...
      if(sddp(iv,i,j).lt.0.001) goto 4
...
#ifdef BINMOD
        ! START MOD write to binary
        ! write 090,000,ver for each timestep
        write(22)((ds(l,i),l=1,3),i=1,ndata)
        ! write edist to stderr
        write(0, '(1x,f10.4)')c(2)
#else
        do l=1,3
        io=21+l
...
C     Optimized version that reuses plans when possible
C     Much faster for repeated calls with same size

      IMPLICIT NONE
      INTEGER NNN, IND
      COMPLEX*8 ACE(NNN)

C     FFTW3 single precision interface
      INCLUDE 'fftw3.f'

C     Static variables to store plans
      INTEGER*8, SAVE :: PLAN_FORWARD = 0
      INTEGER*8, SAVE :: PLAN_BACKWARD = 0
      INTEGER, SAVE :: LAST_SIZE = 0
      COMPLEX*8, ALLOCATABLE, SAVE :: WORK_ARRAY(:)

C     Check if we need to create new plans
      IF (NNN .NE. LAST_SIZE .OR. PLAN_FORWARD .EQ. 0) THEN

C         Clean up old plans if they exist
          IF (PLAN_FORWARD .NE. 0) THEN
...
C         Allocate work array
          ALLOCATE(WORK_ARRAY(NNN))

C         Create new single precision plans
          CALL SFFTW_PLAN_DFT_1D(PLAN_FORWARD, NNN, WORK_ARRAY,
     &                          WORK_ARRAY, FFTW_FORWARD, FFTW_MEASURE)
          CALL SFFTW_PLAN_DFT_1D(PLAN_BACKWARD, NNN, WORK_ARRAY,
     &                          WORK_ARRAY, FFTW_BACKWARD, FFTW_MEASURE)

          LAST_SIZE = NNN
      END IF

C     Copy input data to work array
      WORK_ARRAY(1:NNN) = ACE(1:NNN)

C     Execute appropriate transform
      IF (IND .EQ. 1) THEN
          CALL SFFTW_EXECUTE_DFT(PLAN_FORWARD, WORK_ARRAY, WORK_ARRAY)
      ELSE
          CALL SFFTW_EXECUTE_DFT(PLAN_BACKWARD, WORK_ARRAY, WORK_ARRAY)
      END IF

C     Copy result back
      ACE(1:NNN) = WORK_ARRAY(1:NNN)
